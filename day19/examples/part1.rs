...
fn main() {
    ...
    let result: usize = blueprints
        ...
        .map(|(i, b)| {
            ...
            fn max_of(robots: [Store; 4], f: impl Fn(&Store) -> usize) -> usize {
                robots.iter().map(f).max().unwrap()
            }
            ...
            let geodes = number_of_geodes(
                ...
                TIME_LIMIT,
                Store {
                    ore: max_of(robots, |r: &Store| r.ore),
                    clay: max_of(robots, |r: &Store| r.clay),
                    obsidian: max_of(robots, |r: &Store| r.obsidian),
                    geode: 0,
                },
            );
            geodes * (i + 1)
